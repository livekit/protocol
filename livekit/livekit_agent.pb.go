...
// /agents/{agent_name}/{deployment}/{path} without binding any local listener.
...
// in RegisterWorkerResponse when the worker declares endpoints and the server
...
type AgentHttp_StreamPreamble struct {
	...
	// the caller holds agent-endpoint call permission for this agent and
	// deployment. Always true on a non-public route, since nothing else reaches
	// one; on a public route it separates a caller the project authorized from
	// any holder of a project token, so it is the only bit a worker may gate on.
	// Identity must never be inferred from a header, which is why this lives
...
func (x *AgentHttp_StreamPreamble) GetAuthorized() bool {
	if x != nil {
		return x.Authorized
	}
	return false
