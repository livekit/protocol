...
func NewIOInfoServer(serverID string, svc IOInfoServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (IOInfoServer, error) {
	...
	var err error
	err = psrpc.RegisterHandler(s, "UpdateEgressInfo", nil, svc.UpdateEgressInfo, nil, true, false)
	if err != nil {
		s.Close(false)
		return nil, err
	}

	err = psrpc.RegisterHandler(s, "GetIngressInfo", nil, svc.GetIngressInfo, nil, true, false)
	if err != nil {
		s.Close(false)
		return nil, err
	}

	err = psrpc.RegisterHandler(s, "UpdateIngressState", nil, svc.UpdateIngressState, nil, true, false)
	if err != nil {
		s.Close(false)
		return nil, err
	}
