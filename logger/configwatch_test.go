...
func TestApplyConfigFile(t *testing.T) {
	...
	t.Run("an unchanged malformed file is reported once, not on every poll", func(t *testing.T) {
		...
		var last []byte
		for range 3 {
			last, _ = applyConfigFile(conf, conf.snapshot(), path, last)
		}
		require.Equal(t, 1, logs.FilterMessageSnippet("could not parse log config").Len())
	})

	t.Run("a key a running logger cannot take rejects the file", func(t *testing.T) {
		conf := &Config{Level: "info"}
		path := filepath.Join(t.TempDir(), "logging.yaml")
		writeFile(t, path, "level: debug\njson: true\n")
		...
		_, ok := applyConfigFile(conf, conf.snapshot(), path, nil)
		require.False(t, ok)
		require.Equal(t, "info", conf.Level, "a rejected file must not be half-applied")
		require.False(t, conf.JSON)
	})
...
// Applying config while a running logger reads it: the root leveler resolves the levels through
// ResolveComponentLevel under Config's own lock, and WithItemSampler reads the item sampler
// settings with no lock at all, so the watcher may write the levels and nothing else.
// Meaningful under -race.
func TestApplyConfigFileWhileResolvingComponents(t *testing.T) {
	conf := &Config{Level: "info", ComponentLevels: map[string]string{"pion": "error"}, ItemSampleSeconds: 1}
	l, err := NewZapLogger(conf)
	require.NoError(t, err)
