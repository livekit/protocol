	...
	go func() {
		...
		for {
			...
			case <-done:
				return
			case <-ticker.C:
				if applied, ok := applyConfigFile(conf, path, last); ok {
					last = applied
				}
			}
		}
...
// applyConfigFile pushes path into conf when its bytes differ from last, returning the bytes it
// applied. ok is false when nothing was applied — unreadable, unchanged or invalid — and the
// config already in force stays untouched.
func applyConfigFile(conf *Config, path string, last []byte) (applied []byte, ok bool) {
	data, err := os.ReadFile(path)
	if err != nil {
		// An optional ConfigMap that is not mounted yet is the normal steady state, not something
		// to log once per interval forever.
		return nil, false
	}
